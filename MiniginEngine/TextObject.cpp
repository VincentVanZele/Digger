...
engine::TextObject::TextObject(const std::string& text, std::shared_ptr<Font> font) 
...
void engine::TextObject::Draw() const
{
	if (mTexture != nullptr)
	{
		const auto pos = mTransform.GetPosition();
		Renderer::GetInstance().RenderTexture(*mTexture, pos._x, pos._y);
	}
}
